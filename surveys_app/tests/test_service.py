...
@pytest.mark.django_db
def test_decode_expired_token(draft_survey, settings):
...
def test_decode_tampered_token(draft_survey):
    ...
    # Replace the last 8 signature characters — changing multiple base64 chars
    # guarantees the decoded signature bytes differ and verification always fails.
...
def test_validate_checkbox_rejects_selection_not_in_options():
    ...
    with pytest.raises(ValidationError, match='Prefs'):
        validate_field_value(field, '["A", "D"]')  # D not in options
...
def test_validate_checkbox_options_checked_before_min_selections():
    # D is invalid AND count is below min=2 — options error fires first
...
def test_validate_checkbox_no_options_still_enforces_min_selections():
    ...
    with pytest.raises(ValidationError, match='Prefs'):
        validate_field_value(field, '["A"]')  # only 1 selection, min=2
